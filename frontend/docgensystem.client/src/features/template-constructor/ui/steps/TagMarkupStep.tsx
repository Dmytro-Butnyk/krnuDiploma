    ...
    <div className="h-full min-h-0 overflow-auto pr-1 custom-scrollbar">
      ...
      <div className="grid w-full grid-cols-1 gap-3 md:grid-cols-2">
        {Object.entries(tagTypes).map(([tag, type]) => (
          <div
            key={tag}
            className="grid min-w-0 grid-cols-1 gap-3 rounded-lg border border-blue-100 bg-white p-3 shadow-sm sm:grid-cols-[minmax(0,1fr)_minmax(150px,200px)] sm:items-center"
          >
            <div className="min-w-0">
              <span className={type === 'reserved' ? 'block break-words font-mono text-sm font-black text-slate-400' : 'block break-words font-mono text-sm font-black text-orange-600'}>{`{{${tag}}}`}</span>
            ...
            <select
              ...
              className="w-full min-w-0 rounded-md border border-blue-200 bg-white px-3 py-2 text-sm font-semibold outline-none transition hover:bg-blue-50 focus:border-blue-500 disabled:cursor-not-allowed disabled:border-slate-200 disabled:bg-slate-100 disabled:text-slate-400"
            >
              {type === 'reserved' ? (
