...
type TemplatesRouteMode = 'list' | 'upload' | 'create-constructor' | 'details' | 'edit' | 'generate'

type TemplatesPageProps = {
  routeMode?: TemplatesRouteMode
}
...
const templatesBasePath = '/document-generator/templates'
...
function getTemplateDetailsPath(id: TemplateListItemDto['id']) {
  return `${templatesBasePath}/${id}`
}

function getTemplateEditPath(id: TemplateListItemDto['id']) {
  return `${getTemplateDetailsPath(id)}/edit`
}

function getTemplateGeneratePath(id: TemplateListItemDto['id']) {
  return `${getTemplateDetailsPath(id)}/generate`
}

export function TemplatesPage({ routeMode = 'list' }: TemplatesPageProps) {
  ...
  useEffect(() => {
    ...
    if (routeMode === 'create-constructor') {
      setConstructorMode('create')
      setIsGenerationFormOpen(false)
      setMode(draftTemplate || draftTemplateMeta ? 'constructor' : 'upload')
      return
    }

    if (!templateId || !templatesQuery.data) return

    const routeTemplate = templatesQuery.data.find((template) => sameTemplateId(template.id, templateId))
    if (!routeTemplate) {
      setSelectedTemplate(null)
      setMode('list')
      navigate(templatesBasePath, { replace: true })
      return
    }

    setSelectedTemplate(routeTemplate)
    setMenuTemplateId(null)
    setConstructorMode(routeMode === 'edit' ? 'edit' : 'create')
    setConstructorTemplateName(routeMode === 'edit' ? routeTemplate.name.replace(/\.docx$/i, '') : '')
    setIsGenerationFormOpen(routeMode === 'generate')
    setMode(routeMode === 'edit' ? 'constructor' : 'details')
  }, [draftTemplate, draftTemplateMeta, navigate, routeMode, templateId, templatesQuery.data])
  ...
  }, [navigate, selectedTemplate, templatesQuery.data])
  ...
  const openDetails = (template: TemplateListItemDto) => {
    ...
    setIsGenerationFormOpen(false)
    setGenerationParams({})
    navigate(getTemplateDetailsPath(template.id))
  }
  ...
  const openUpload = () => {
    ...
    setErrorText(null)
    setMode('upload')
    navigate(`${templatesBasePath}/new`)
  }
  ...
  const openConstructorForEdit = (template: TemplateListItemDto) => {
    ...
    setErrorText(null)
    setMode('constructor')
    navigate(getTemplateEditPath(template.id))
  }
  ...
  const leaveConstructor = () => {
    ...
    navigate(constructorMode === 'create' ? `${templatesBasePath}/new` : selectedTemplate ? getTemplateDetailsPath(selectedTemplate.id) : templatesBasePath)
  }
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      setConstructorMode('create')
      setMode('constructor')
      navigate(`${templatesBasePath}/new/constructor`)
    } catch (error) {
  ...
  const handleUpdateTemplate = async (configuration: TemplateConfiguration) => {
    ...
    try {
      ...
      setMode('details')
      navigate(getTemplateDetailsPath(selectedTemplate.id))
  ...
  const openGenerationForm = () => {
    ...
    if (selectedTemplate) {
      navigate(getTemplateGeneratePath(selectedTemplate.id))
    }
  }
            ...
            {filteredTemplates.map((template) => (
              <TemplateRow
                ...
                onGenerate={() => {
                  ...
                  setIsGenerationFormOpen(true)
                  navigate(getTemplateGeneratePath(template.id))
                }}
                onConfigure={() => {
